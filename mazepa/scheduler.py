...
class Scheduler:
    def __init__(
        self, queue_name=None, completion_queue_name=None, queue_region=None, threads=1
    ):
        self.queue = Queue(
    ...
    def register_job(self, job, job_name=None):
        ...
        if not isinstance(job, Job):
            if not inspect.isgeneratorfunction(job) and not inspect.isgenerator(job):
                raise Exception(
            ...
            if job_name in self.unfinished_jobs:
                raise Exception(
                    'Unable to register task "{}": \
                        task with name "{}" is already registerd'.format(
                        job_name, job_name
                    )
                )
