    ...
    def execute(self):
        ...
        if self.completion_queue_name is not None:
            if self.job_id is None:
                raise Exception("'job_id' property must be set " "for each task")
...
def send_completion_report(queue_name, queue_region, task_id, job_id):
    completion_queue = TaskQueue(
        queue_name=completed_queue_name, region_name=queue_region, n_threads=0
    )
    ...
    message = [
        {
            ...
            "MessageBody": json.dumps({"task_id": task_id, "job_id": job_id}),
        }
    ]
...
def send_message(queue_api_obj, queue_sqs_obj, message):
    msg_ack = queue_sqs_obj.send_message_batch(
        QueueUrl=queue_api_obj._qurl, Entries=message
    )
    ...
    if success == False:
        raise ValueError(
            f"Failed to send message {message} to " f"queue {queue_api_obj._qurl}"
        )
...
class Queue:
    def __init__(
        ...
        if queue_name is None:
            ...
            self.queue = taskqueue.GreenTaskQueue(queue_name, region=queue_region)
            self.queue_boto = boto3.client("sqs", region_name=queue_region)
            self.queue_url = self.queue_boto.get_queue_url(QueueName=self.queue_name)[
                "QueueUrl"
            ]

            if completion_queue_name is not None:
                self.completion_registry = None
                self.completion_queue = taskqueue.GreenTaskQueue(
                    completion_queue_name, region=queue_region
                )
    ...
    def remote_queue_is_empty(self):
        ...
        for i in range(3):
            response = self.queue_boto.get_queue_attributes(
                QueueUrl=self.queue_url, AttributeNames=attribute_names
            )
        ...
        while True:
            ...
            if "Messages" not in resp:
                ...
                for i in range(len(resp["Messages"])):
                    ...
                    completed_tasks.append(json.loads(resp["Messages"][i]["Body"]))

                self.completion_queue_boto.delete_message_batch(
                    QueueUrl=self.completion_queue_url, Entries=entries
                )
